    ...
    panic::set_hook(Box::new(move |panic_info| {
        ...
        for (i, frame) in backtrace.frames().iter().enumerate() {
            ...
            for symbol in frame.symbols() {
                let name = symbol.name().map(|n| n.to_string()).unwrap_or_else(|| "<unknown>".into());

                let file = symbol.filename().map(|p| p.display().to_string()).unwrap_or_else(|| "<unknown>".into());

                let line = symbol.lineno().unwrap_or(0);

                writeln!(out, "  #{:<2} {} @ [{:?}:{}]", i, name, Path::new(&file).file_name().unwrap(), line).unwrap();
            }
        }
