...
macro_rules! building_config {
    ($config_struct:ident) => {
        impl $crate::building::config::BuildingConfig for $config_struct {
            #[inline]
            fn building_kind(&self) -> $crate::building::BuildingKind {
                self.kind
            }

            #[inline]
            fn archetype_kind(&self) -> $crate::building::BuildingArchetypeKind {
                self.kind.archetype_kind()
            }

            #[inline]
            fn tile_def_name_hash(&self) -> StringHash {
                debug_assert!(self.tile_def_name_hash != common::hash::NULL_HASH);
...
engine::configurations! { BUILDING_CONFIGS_SINGLETON, BuildingConfigs, "buildings/configs" }
