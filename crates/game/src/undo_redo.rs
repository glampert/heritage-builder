...
macro_rules! game_object_undo_redo_state {
    (
        ...
        impl $struct_name {
            fn new_state(instance: $struct_name) -> Option<Box<dyn $crate::undo_redo::GameObjectSavedState>> {
        ...
        for state in &self.saved_states {
            ...
            if let Some(object_tile) = maybe_object_tile {
                if let Some(game_object_state) = &state.game_object_state {
                    if let Some(game_object) = context.find_game_object_for_tile_mut(object_tile) {
...
// Global instance:
common::singleton_late_init! { UNDO_REDO_SINGLETON, UndoRedoSingleton }
